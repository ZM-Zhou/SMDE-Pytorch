...
class ResnetEncoderMatching(nn.Module):
    ...
    def __init__(self, num_layers, pretrained, input_height, input_width,
        ...
        # set min and max depth bins as parameters
        self.min_depth_bin = nn.Parameter(torch.Tensor([min_depth_bin]))
        self.max_depth_bin = nn.Parameter(torch.Tensor([max_depth_bin]))
    ...
    def forward(self, current_image, lookup_images, poses, K, invK,
        ...
        with torch.no_grad():
            if self.adaptive_bins:
                self.compute_depth_bins(self.min_depth_bin.cpu().numpy()[0],
                                        self.max_depth_bin.cpu().numpy()[0])
